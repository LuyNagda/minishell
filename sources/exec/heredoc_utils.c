...
/*   Updated: 2024/01/29 18:13:16 by lunagda          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	here_doc_error_handling(t_minishell *shell, t_commands *command, t_pipex *pipex)
{
...
void	here_doc(t_minishell *shell, t_commands *command, t_pipex *pipex)
{
	...
	if (has_heredoc(command, "<<") && command->arguments_amount != 1)
	{
		...
		while (ft_strncmp(command->here_doc, line, ft_strlen(command->here_doc)))
		{
